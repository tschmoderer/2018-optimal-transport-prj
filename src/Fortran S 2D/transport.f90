program transport
    ...
    integer :: i, k, l 
  	...
  	double precision, dimension(P+1,N+1,Q+1,3) :: V, IU
  	double precision, dimension(P+1,N+1,Q+1) :: D, dU
    double precision, dimension(P+3,N+3,Q+3) :: Au, rA
    ...
    print *, 'Interp adjoint     ', sum(U*ISV), sum(V*IU), sum(U*ISV) - sum(V*IU)
    ...
    print *, 'Divergence adjoint ', sum(U*dSD), sum(D*dU),  sum(U*dSD) - sum(D*dU)
	...
	print *, 'Opérateur A adjoint', sum(U*ASrA), sum(Au*rA), sum(U*ASrA) - sum(Au*rA)
...
!! Divergence 
	...
	implicit none
		...
		D =     (N+1)*(U(1:P+1,2:N+2,1:Q+1,1) - U(1:P+1,1:N+1,1:Q+1,1))
		D = D + (P+1)*(U(2:P+2,1:N+1,1:Q+1,2) - U(1:P+1,1:N+1,1:Q+1,2))
		D = D + (Q+1)*(U(1:P+1,1:N+1,2:Q+2,3) - U(1:P+1,1:N+1,1:Q+1,3))
	end function div	
